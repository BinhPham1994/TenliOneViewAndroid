...
class NotifyConfigViewModel(
    ...
    fun updateMasterNotify(enabled: Boolean) {
        ...
        viewModelScope.launch {
            try {
                val request = com.tenli.oneview.model.network.UpdateNotifySettingRequest(
                    notification = com.tenli.oneview.model.network.NotifyEnabledConfig(enabled = enabled)
                )
    ...
    fun updateGroupEnabled(groupKey: String?, enabled: Boolean) {
        ...
        viewModelScope.launch {
            try {
                val request = com.tenli.oneview.model.network.UpdateNotifyGroupRequest(
                    key = groupKey,
                    notification = com.tenli.oneview.model.network.NotifyConfigPayload(enabled = enabled)
                )
    ...
    fun updateAlarmMode(newMode: String) {
        ...
        viewModelScope.launch {
            try {
                ...
                val request = com.tenli.oneview.model.network.UpdateNotifyGroupRequest(
                    key = typeKey,
                    notification = com.tenli.oneview.model.network.NotifyConfigPayload(
                        enabled = newMode != "none",
                        alarmMode = com.tenli.oneview.model.network.AlarmModePayload(default = newMode)
                    )
                )
