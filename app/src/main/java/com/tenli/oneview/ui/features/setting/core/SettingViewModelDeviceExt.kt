...
fun SettingViewModel.controlBoxSystem(command: String, successMessage: String) {
    ...
    viewModelScope.launch {
        try {
            ...
            val request = com.tenli.oneview.model.network.DeviceCommandRequest(command = command)
    ...
    viewModelScope.launch {
        ...
        try {
            ...
            if (cloudRes.isSuccess) {
                try {
                    val request = com.tenli.oneview.model.network.ConfigHttpKeyRequest(httpKey = newKey)
    ...
    viewModelScope.launch {
        try {
            ...
            if (result.isSuccess) {
                if (isResetLocal) {
                    try {
                        val request = com.tenli.oneview.model.network.DeviceCommandRequest(command = "reset")
...
fun SettingViewModel.controlAlarm(command: String, duration: Long? = null) {
    ...
    viewModelScope.launch {
        try {
            val request = com.tenli.oneview.model.network.DeviceCommandRequest(command = command, duration = duration)
            val result = boxRepository.controlDevice(device.publicTargetURI, device.key, request)
            if (result.isSuccess) {
                val msg = when {
